export const SERVER_URL = "http://cerebro-back:8080";

export const API_ADDITIONAL_SERVICES = SERVER_URL + "/api/additionalServices";
export const API_ADVERTISEMENT = SERVER_URL + "/api/advertisement";
export const API_CATEGORIES = SERVER_URL + "/api/categories";
export const API_CURRENT_USER = SERVER_URL + "/api/user";
export const API_IMAGE = SERVER_URL + "/api/image";
export const API_ORDER = SERVER_URL + "/api/order"
export const API_REGISTRATION = SERVER_URL + "/api/registration";
export const API_TYPES = SERVER_URL + "/api/types";
export const API_USER = SERVER_URL + "/api/user";
export const LOGIN = SERVER_URL + "/api/auth";
export const LOGOUT = SERVER_URL + "/api/logout";


